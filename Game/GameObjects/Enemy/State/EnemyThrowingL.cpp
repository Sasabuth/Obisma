...
void EnemyThrowingL::Update(float elapsedTime)
{
	...
	if (!m_isThowing)
	{
		...
		if (m_animation->GetAnimTime() > 0.58f)
		{
			...
			ball->SetVelocity(SimpleMath::Vector3::Transform(SimpleMath::Vector3::UnitX, m_enemy->GetRotation() * rotate) * Enemy::BALL_SPEED);
...
void EnemyThrowingL::SetBallPosition(Ball* ball, DirectX::SimpleMath::Matrix handMatrix)
{
